...
def find_date_dirs_under_mobile(base: Path):
    mobile = base / "mobile"
    if not mobile.exists() or not mobile.is_dir():
        return []
    return [p for p in mobile.iterdir() if p.is_dir() and DATE_DIR_RE.match(p.name)]
...
def main():
    ap = argparse.ArgumentParser(
        description="Reset ./mobile/YYYY-MM-DD/ subfolders to contain only index.html that redirects via puzzle_date."
    )
    ...
    targets = find_date_dirs_under_mobile(base)

    if not targets:
        print(f"No YYYY-MM-DD directories found under {base / 'mobile'}.")
        return
